...
def create_rule(payload: RuleCreate, db: Session = Depends(get_db)) -> RuleOut:
    ...
    db.add(rule)
    db.flush()
    write_audit(db, entity_table="categorization_rules", entity_id=rule.id, action="create", actor="controller")
    db.commit()
    db.refresh(rule)
    return rule
...
def update_rule(rule_id: int, payload: RuleCreate, db: Session = Depends(get_db)) -> RuleOut:
    rule = db.get(CategorizationRule, rule_id)
    if not rule:
        raise HTTPException(404, "Rule not found")
    for k, v in payload.model_dump().items():
        setattr(rule, k, v)
    write_audit(db, entity_table="categorization_rules", entity_id=rule.id, action="update", actor="controller")
    db.commit()
    db.refresh(rule)
    return rule
...
def delete_rule(rule_id: int, db: Session = Depends(get_db)) -> dict:
    ...
    if not rule:
        raise HTTPException(404, "Rule not found")
    write_audit(db, entity_table="categorization_rules", entity_id=rule.id, action="delete", actor="controller")
    db.delete(rule)
    db.commit()
